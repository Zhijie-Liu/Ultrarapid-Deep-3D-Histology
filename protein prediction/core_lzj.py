...
class L2PTissueClearing(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img3 = []
        if self.transform:
            img1 = self.transform(np.array(img, dtype=np.float32))
            img2 = img1.to(torch.float32) / 65535
...
class Lipid2Protein(Dataset):
    def __init__(self, lipid_data_path, protein_data_path, transform):
        self.lipid_list = []
        lipid_sub = each_dir(lipid_data_path)
        lipid_sub.sort()
        for sub in lipid_sub:
            list = each_img_specify(sub, '.tif')
            list.sort()
            self.lipid_list.extend(list)
        self.protein_list = []
        protein_sub = each_dir(protein_data_path)
        protein_sub.sort()
        for sub in protein_sub:
            list = each_img_specify(sub, '.tif')
            list.sort()
            self.protein_list.extend(list)
        self.transform = transform
        self.normalize = transforms.Normalize((0.5), (0.5))

    def __len__(self):
        return len(self.lipid_list)

    def __getitem__(self, index):
        lipid_path = self.lipid_list[index]
        protein_path = self.protein_list[index]
        lipid = Image.open(lipid_path)
        protein = Image.open(protein_path)
        lipid2 = []
        protein2 = []
        if self.transform:
            lipid, protein = self.transform(lipid, protein)
            lipid1 = lipid.to(torch.float)/4095
            protein1 = protein.to(torch.float)/4095

            lipid2 = self.normalize(lipid1)
            protein2 = self.normalize(protein1)
        return lipid2, protein2
