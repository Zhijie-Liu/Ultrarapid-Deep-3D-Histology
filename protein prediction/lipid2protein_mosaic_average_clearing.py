...
def get_probability_matrix(net, cuda, nrow, ncol, img_mosaic):
    target = np.zeros(shape=(nrow * img_size, ncol * img_size))
    for i in range(0, nrow - step + 1):
        for j in range(0, ncol - step + 1):
            ...
            img_tensor = (torch.from_numpy(img_temp.astype(np.float32)).float() / 65535 * 2 - 1).unsqueeze(0).unsqueeze(0)
            # img = transform(Image.fromarray(img_temp)).unsqueeze(0)
            if torch.cuda.is_available():
                img_tensor = img_tensor.to(cuda)
            output = net(img_tensor)
            fake_img = torch.clip(output[0][0].detach().cpu(), -1, 1)
...
if __name__ == '__main__':
    select = 0
    if select == 0:
        img_path = '20250220/G5 map_ave3_size1024_net100 10-120 10 290 poisson mosaic'
        # net_path = core_lzj.get_file()
        net_path = 'time_20240311183146_lipid2protein networks size512/lipid2protein networks size512_epochs_100.pkl'
    ...
    G.load_state_dict(torch.load(net_path, map_location='cuda:' + gpu.__str__())['G'])
    if torch.cuda.is_available():
        G.to(device)
    # print('current net is ' + net_name.__str__())
    G.eval()

    img_sub_path = os.path.join(img_path, 'protein')
    core_lzj.check_folder_existence(img_sub_path)
    for img_dir in img_list:
        print(img_dir)
        img_name = os.path.basename(img_dir).split('.')[0]
        img_raw = cv2.imread(img_dir, cv2.IMREAD_ANYDEPTH)
        ...
        target = get_probability_matrix(net=G, cuda=device, nrow=img_nrow, ncol=img_ncol, img_mosaic=img_raw)
        dim_row = np.repeat(np.concatenate((np.arange(1, step, 1), np.ones(img_nrow - (step - 1) * 2) * step, np.arange(step - 1, 0, -1))), img_size)
        dim_col = np.repeat(np.concatenate((np.arange(1, step, 1), np.ones(img_ncol - (step - 1) * 2) * step, np.arange(step - 1, 0, -1))), img_size)
        ...
        target_uint16 = (((target/adjust_matrix)/2+0.5)*65535).astype(np.uint16)
        img = Image.fromarray(target_uint16)

        img_save_path = os.path.join(img_sub_path, img_name + '_protein.tif')
