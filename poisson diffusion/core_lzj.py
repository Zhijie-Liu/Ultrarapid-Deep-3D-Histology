...
class DDPMFolder16bit1024ResizeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img0 = (torch.tensor(img_resize.astype(np.float32))/65535).unsqueeze(0)
        img3 = self.normalize(img0)
...
class DDPMFolder16bitDataset(Dataset):
    def __init__(self, path, transform):
        self.path = path
        self.transform = transform
        self.img_dir = each_dir(path)
    ...
    def __getitem__(self, index):
        ...
        img = cv2.imread(img_path, cv2.IMREAD_ANYDEPTH)
        if self.transform:
            img0 = (torch.tensor(img.astype(np.float32))/65535).unsqueeze(0)
            img3 = self.normalize(img0)
