...
class HE2dtwoclassTestset(Dataset):
    def __init__(self, HE_path, mask_path=None):
        self.HE_path = HE_path
        self.HE_dir = each_img_specify(HE_path, ['.tif'])
        self.HE_dir.sort()
        if mask_path:
            self.mask_path = mask_path
            self.mask_dir = each_img_specify(mask_path, ['.tif'])
            self.mask_dir.sort()
            self.data_list = self.HE_dir + self.mask_dir
        else:
            self.data_list = self.HE_dir
    ...
    def __getitem__(self, index):
        HE_path = self.data_list[index]
        HE = cv2.cvtColor(cv2.imread(HE_path), cv2.COLOR_BGR2RGB)
        HE_tensor = torch.tensor(HE.astype(np.float32))/255
        HE_tensor1 = HE_tensor.permute(2,0,1)
        if "normal" in HE_path:
            label = torch.tensor(0)
        else:
            label = torch.tensor(1)
        return HE_tensor1, label


class HE2dtwoclassgroupDataset(Dataset):
    def __init__(self, HE_path, mask_path):
        self.HE_dir = each_dir(HE_path)
        self.HE_dir.sort()
        self.HE_list = []
        for dir in self.HE_dir:
            temp = each_img_specify(dir, ['.tif'])
            temp.sort()
            self.HE_list.extend(temp)
        self.mask_dir = each_dir(mask_path)
        self.mask_dir.sort()
        self.mask_list = []
        for dir in self.mask_dir:
            temp = each_img_specify(dir, ['.tif'])
            temp.sort()
            self.mask_list.extend(temp)
        self.data_list = self.HE_list + self.mask_list
...
class HE3dtwoclassTestset(Dataset):
    def __init__(self, path1, path2=None):
        self.path1 = path1
        self.path1_dir = each_img_specify(path1, ['.tif'])
        self.path1_dir.sort()
        if path2:
            self.path2 = path2
            self.path2_dir = each_img_specify(path2, ['.tif'])
            self.path2_dir.sort()
            self.data_list = self.path1_dir + self.path2_dir
        else:
            self.data_list = self.path1_dir

    def __len__(self):
        return len(self.data_list)

    def __getitem__(self, index):
        path1 = self.data_list[index]
        img = tifffile.imread(path1)
        img_tensor = torch.tensor(img.astype(np.float32)) / 255
        img_tensor1 = img_tensor.permute(3, 0, 1, 2)
        ...
        return img_tensor1, label


class HE3dtwoclassgroupDataset(Dataset):
    def __init__(self, path1, path2):
        self.path1 = each_dir(path1)
        self.path1.sort()
        self.path1_list = []
        for dir in self.path1:
            temp = each_img_specify(dir, ['.tif'])
            temp.sort()
            self.path1_list.extend(temp)
        self.path2 = each_dir(path2)
        self.path2.sort()
        self.path2_list = []
        for dir in self.path2:
            temp = each_img_specify(dir, ['.tif'])
            temp.sort()
            self.path2_list.extend(temp)
        self.data_list = self.path1_list + self.path2_list
...
class HE3dtwoclassDataset(Dataset):
    def __init__(self, HE_path, mask_path):
        self.HE_path = HE_path
        self.HE_dir = each_img_specify(HE_path, ['.tif'])
        self.HE_dir.sort()
    ...
    def __getitem__(self, index):
        ...
        HE_tensor = torch.tensor(HE.astype(np.float32))/255
        HE_tensor1 = HE_tensor.permute(3,0,1,2)
...
class HE3dDataset(Dataset):
    def __init__(self, HE_path, mask_path, threshold):
        self.HE_path = HE_path
        self.HE_dir = each_img_specify(HE_path, ['.tif'])
        self.HE_dir.sort()
    ...
    def __len__(self):
        return len(self.HE_dir)

    def __getitem__(self, index):
        HE_path = self.HE_dir[index]
        HE = tifffile.imread(HE_path)
        HE_tensor = torch.tensor(HE.astype(np.float32))/255
        HE_tensor1 = HE_tensor.permute(3,0,1,2)
        ...
        mask_sum_ave = mask.sum()/120/120/10
        if mask_sum_ave < self.threshold:
            label = torch.tensor(0)
        else:
            label = torch.tensor(1)
...
class Lectin3dDataset_01(Dataset):
    def __init__(self, path):
        ...
        # self.img_list = each_img_specify(path, '.tif')
        # self.img_list.sort()
...
class DDPMFolderDataset(Dataset):
    def __init__(self, path, transform):
        self.path = path
        self.transform = transform
        self.img_dir = each_dir(path)
    ...
    def __getitem__(self, index):
        ...
        # img = Image.open(img_path)
        if self.transform:
            img0 = self.transform(img_pil)
            img1 = img0.to(torch.float)
            img2 = img1 / 4095
            img3 = self.normalize(img2)


        return img3


class DDPMFolderDatasetFinetune(Dataset):
    def __init__(self, path, transform):
        self.path = path
        self.transform = transform
        self.img_dir = each_dir(path)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(img_path)
        label = img_path.replace(r'\\', '/').split('/')[-3]
        label = eval(label)
        if self.transform:
            img = self.transform(img)
        return img, label
...
def resize_image_itk(itkimage, newSize, resamplemethod=sitk.sitkNearestNeighbor):
    ...
    newSize = newSize.astype(np.int16) # spacing格式转换
