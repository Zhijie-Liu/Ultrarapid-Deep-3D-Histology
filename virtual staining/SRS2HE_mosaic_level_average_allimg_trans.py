...
def get_probability_matrix(net, cuda, nrow, ncol, img_mosaic):
    ...
    adjust_matrix = np.zeros(shape=(nrow * img_size, ncol * img_size, 3))
    for i in range(0, nrow - step + 1):
        for j in range(0, ncol - step + 1):
            ...
            adjust_matrix[i * img_size: i * img_size + net_img_size, j * img_size: j * img_size + net_img_size, :] += 1

            print(i, j)

    return target, adjust_matrix


if __name__ == '__main__':
    select = 0
    if select == 0:
        img_path = 'new0122/intra 1707 1041'
        # net_path = core_lzj.get_file()
        net_path = 'virtual staining/united network/mapping.pkl'
    ...
    img_sub_path = os.path.join(img_path, 'mapping128')
    ...
    for img_dir in img_list:
        ...
        target, adjust_matrix = get_probability_matrix(net=G_A, cuda=device, nrow=img_nrow, ncol=img_ncol, img_mosaic=img_raw)
        ...
        target_uint8 = (target/adjust_matrix).astype(np.uint8)
